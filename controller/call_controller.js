...
const VideoVoiceCallReject = async (request, response) => {
    ...
    try {
        ...
        connection.query(query1, values1, async (err, result) => {
            ...
            const selectquery = "SELECT user_id, other_user_id FROM video_call_master WHERE video_call_id = ? AND delete_flag = 0 ";
            const selectvalues = [video_call_id];
            connection.query(selectquery, selectvalues, async (err, selectresult) => {
                if (err) {
                    return response.status(200).json({ success: false, msg: languageMessage.internalServerError, key: err.message });
                ...
                const callData = selectresult[0];
                let other_user_id;

                if (user_id === callData.user_id.toString()) {
                    other_user_id = callData.other_user_id;
                } else if (user_id === callData.other_user_id.toString()) {
                    other_user_id = callData.user_id;
                } else {
                    return response.status(200).json({ success: false, msg: "User not part of this call." });
                }
